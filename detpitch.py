    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if webcam:  # batch_size >= 1
                p, im0, imgg = path[i], im0s[i].copy(), dataset.count
                s += f'{i}: '
            else:
                p, im0, imgg = path, im0s.copy(), getattr(dataset, 'imgg', 0)

            p = Path(p)  # to Path
            save_path = str(save_dir / p.name)  # im.jpg
            txt_path = str(save_dir / 'labels' / p.stem) + ('' if dataset.mode == 'image' else f'_{imgg}')  # im.txt
            ...
            if len(det):
                ...
                lower_red = np.array([22, 58, 172])
                upper_red = np.array([33, 98, 255])
                ...
                imgResult = cv2.bitwise_and(imgg,imgg,mask=mask)
                contours, hierarchy = cv2.findContours(mask.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                output = cv2.drawContours(imgResult, contours, -1, (0, 0, 255), 2)
                
            # dividing pitch 
                for c in contours:
                    if cv2.contourArea(c) <=  19900 :
                        continue    
                    ...
                    cv2.rectangle(im0, (x, y+int(h*0.1)), (x + w, y + int(h*0.3)), (255, 0,0), 2)#full
                    ful=[x,y + int(h*0.1),x+w,y + int(h*0.3)]
                    
                    cv2.rectangle(im0, (x, y + int(h*0.3)), (x + w, y + int(h*0.4)), 	(0,0,255), 2)#good
                    good=[x,y + int(h*0.3),x+w,y + int(h*0.4)]
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_txt:  # Write to file
                        ...
                        # print((torch.tensor(xywh).view(1,4)).tolist()[0])
